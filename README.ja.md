...
- 複数PDFのページを1つのPDFへ結合
- JPG / PNG / WebPを1ページとしてPDFと混在追加
- PowerPoint（`.pptx`）をスライド単位で追加・並び替え
- PowerPoint出力は選択可能なUnicodeテキスト層を保持し、対応する基本SVG図形はベクターで重ねて出力
- 画像ごとに `Fit` / `Fill` / `Original Size` を選択
- 全ページまたは選択ページだけを保存（スマホでは選択時だけ専用ボタンを1段バー内に表示）
- 元に戻す・やり直す
...
- パスワード付きPDFは、正しいパスワードを入力すると開けます。編集後のPDFはパスワードなしで保存されます。
